...
def transform_php_test(content: str, original_class: str) -> Tuple[str, str]:
    ...
    # Add require_once
    content = re.sub(
        r'use PHPUnit\\Framework\\TestCase;',
        f'use PHPUnit\\Framework\\TestCase;\n\nrequire_once __DIR__ . \'/{refactored_class}.php\';',
        content
    )
    
    # Change class name
    content = re.sub(
        rf'class {original_class}Test extends TestCase',
        f'final class {solution_test_class} extends TestCase',
        content
    )
    
    # Change property type
    content = re.sub(
        rf'private {original_class} \$processor;',
        f'private {refactored_class} $processor;',
        content
    )
    
    # Change instantiation
    content = re.sub(
        rf'new {original_class}\(\)',
        f'new {refactored_class}()',
        content
    )

...
class PodcastThing {
    ...
    public async doStuff(command: string, urlOrPodcastId: string, extraParam: string | null = null): Promise<void> {
        ...
        if (command === 'add') {
            ...
            } catch (e) { // Exceptions sind lästig, einfach eine Nachricht ausgeben.
                console.log("Irgendwas ist schiefgelaufen beim Hinzufügen.", e.message);
            }

        } else if (command === 'download') {
            console.log("Suche nach Episoden zum Herunterladen...");
            const podcastToDownload = this.data.podcasts.find(p => String(p.id) === String(urlOrPodcastId));
            ...
            if (podcastToDownload) {
                ...
                const tempEpisodes: any[] = []; // Schlechter Variablenname
    ...
    public handleThings(action: string, formatType: string = 'plain'): void {
        if (action === 'list') {
            ...
            for (const eThing of this.data.episodes) {
                const pTitle = this.data.podcasts.find(p => p.id === eThing.podcast_id)?.title || "Unbekannt";
    ...
    public processData(someId: number, newTag: string): void {
        ...
        const p = this.data.podcasts.find(p => p.id === someId);
        if (p) {
            p.tags.push(newTag);
    ...
    public makeItWork(): void {
        ...
        this.data.episodes = this.data.episodes.filter(item => fs.existsSync(item.file));
...
(async () => {
    ...
    if (args.length === 0) {
        ...
        console.log("  npx ts-node podcast_mess.ts add <podcast_rss_url>");
        console.log("  npx ts-node podcast_mess.ts list");
        console.log("  npx ts-node podcast_mess.ts download <podcast_id>");
        console.log("  npx ts-node podcast_mess.ts export <json|txt>");
        console.log("  npx ts-node podcast_mess.ts tag <podcast_id> <tag_name>");
        console.log("  npx ts-node podcast_mess.ts cleanup");
        exit(1);
    }
